...
def load_model(args):
    ...
    model = SmallResCNNv5(num_classes=num_classes, base_channels=cfg["base_channels"], dropout_p=cfg["dropout_p"]).to(device)

    model_path = Path(f"{MODELS_PATH}/best_model_experiment_resnet18_{args.target_class}_SmallResCNNv5_exp_{args.experiment}.pth")
